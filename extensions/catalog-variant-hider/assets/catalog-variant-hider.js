(function () {
  ...
  async function init() {
    ...
    if (singleProductId) {
      console.log(`[CVH] Fetching rules for Product: ${singleProductId}`);
      const rules = await fetchRules(resolvedLocationId, singleProductId);
      console.log("[CVH] API Response:", rules);
      
      if (rules.debug) {
          console.log(`[CVH] Server State | Version: ${rules.debug.version} | Catalog: ${rules.debug.resolvedCatalogId} (${rules.debug.ruleName || 'None'}) | Strategy: ${rules.debug.strategy}`);
      }

      const applyAll = () => {
        const SELECTORS = "#main-product, .product, .product-single, .card, .grid__item, .product-section, .product-item, .product__info-container";
