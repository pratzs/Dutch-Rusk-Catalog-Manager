...
export async function loader({ request }) {
  ...
  try {
    ...
    overrides.forEach((o) => {
      ...
      // Add the catalog name (or ID if name is missing)
    ...
    // 3. Fetch Product Titles from Shopify (Batch query up to 250 items)
    ...
    // 4. Format as an array for the UI table
...
export default function AuditReport() {
  ...
  const downloadCSV = () => {
    ...
    const rows = report.map(item => [
      `"${item.title.replace(/"/g, '""')}"`, // Escape quotes for CSV
            ...
            <s-block-stack gap="base">
              ...
              <s-stack direction="inline" style={{ justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
                <div>
                  ...
                  <s-text color="subdued">A bird's-eye view of every product with active visibility restrictions across all catalogs.</s-text>
                </div>
              ...
              {error && <s-banner tone="critical">Error loading report: {error}</s-banner>}
