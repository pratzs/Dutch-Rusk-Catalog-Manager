// Keeps every collection arranged by brand, in the admin, on a schedule.
//
// WHY THIS EXISTS
...
// server sends them already grouped and nothing moves after paint.
//
// THE ORDER
...
// so it sits out of position until this runs again. Hence the schedule.
...
const LEAVE_ALONE = ["New Arrivals"];
 ...
 * Pure, so it can be tested without touching Shopify.
 */
  ...
  return async (query, variables = {}) => {
    for (let attempt = 0; attempt < 10; attempt++) {
      ...
      if (json.errors) {
        ...
        // Shopify throttles hard on a job this size; back off rather than fail.
...
async function arrangeCollection(gql, col) {
  ...
  // Nothing new has landed and nothing has shifted, so leave it completely
  // alone. This is what makes the scheduled run cheap on a quiet day.
  ...
  // Read the storefront order back rather than trusting the mutation.
...
async function runBrandOrder(shop, accessToken) {
  ...
  for (const col of targets) {
    try {
      ...
      if (r.positionsWrong) console.error(`[brand-order] ${col.title}: ${r.positionsWrong} position(s) did not stick`);
    } catch (e) {
  ...
  console.log(
    `[brand-order] ${targets.length} collection(s): ${changed.length} rearranged, ${untouched} already correct, ${failed.length} failed`
  );
  return { collections: targets.length, rearranged: changed.length, alreadyCorrect: untouched, failed };
}
